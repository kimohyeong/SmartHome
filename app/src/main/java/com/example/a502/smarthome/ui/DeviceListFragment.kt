package com.example.a502.smarthome.ui
...
import com.example.a502.smarthome.DevicesLoader
import com.google.android.material.snackbar.Snackbar
import com.example.a502.smarthome.DevicesLoader.DevicesLoadResult
import com.example.a502.smarthome.R
...
class DeviceListFragment : Fragment(), LoaderManager.LoaderCallbacks<DevicesLoader.DevicesLoadResult> {
    ...
    private var deviceSetupCompleteReceiver: ParticleDeviceSetupLibrary.DeviceSetupCompleteReceiver? = null
    ...
    override fun onCreateView(
        ...
        adapter = DeviceListAdapter(Objects.requireNonNull<FragmentActivity>(activity))
        rv.adapter = adapter
    ...
    override fun onCreateLoader(i: Int, bundle: Bundle?): Loader<DevicesLoader.DevicesLoadResult> {
        return DevicesLoader(activity)
    }

    override fun onLoadFinished(loader: Loader<DevicesLoader.DevicesLoadResult>, result: DevicesLoader.DevicesLoadResult) {
        refresh_layout.isRefreshing = false
    ...
    override fun onLoaderReset(loader: Loader<DevicesLoader.DevicesLoadResult>) {
        // no-op
    }
    ...
    private fun addSparkCoreDevice() {
        try {
            ...
            var intent =
                    Objects.requireNonNull<FragmentActivity>(activity).packageManager.getLaunchIntentForPackage(
                            coreAppPkg
                    )
            if (intent == null) {
    ...
    internal class DeviceListAdapter(private val activity: FragmentActivity) :
        ...
        private val devices = Py.list<ParticleDevice>()
        private val filteredData = Py.list<ParticleDevice>()
        private var defaultBackground: Drawable? = null
        var textFilter: String? = ""
            private set
        private var typeFilters: List<ParticleDevice.ParticleDeviceType?> = Arrays.asList(
                *ParticleDevice.ParticleDeviceType.values()
        )
        ...
        override fun onBindViewHolder(holder: ViewHolder, position: Int) {
            ...
            val (modelNameRes, productImageRes) = when (device.deviceType) {
                ParticleDevice.ParticleDeviceType.CORE -> Pair(R.string.core, R.drawable.core_vector)
                ParticleDevice.ParticleDeviceType.PHOTON -> Pair(R.string.photon, R.drawable.photon_vector_small)
                ParticleDevice.ParticleDeviceType.ELECTRON -> Pair(R.string.electron, R.drawable.electron_vector_small)
                ParticleDevice.ParticleDeviceType.RASPBERRY_PI -> Pair(R.string.raspberry, R.drawable.pi_vector)
                ParticleDevice.ParticleDeviceType.P1 -> Pair(R.string.p1, R.drawable.p1_vector)
                ParticleDevice.ParticleDeviceType.RED_BEAR_DUO -> Pair(R.string.red_bear_duo, R.drawable.red_bear_duo_vector)
                ParticleDevice.ParticleDeviceType.ARGON,
                ParticleDevice.ParticleDeviceType.A_SERIES -> Pair(R.string.product_name_argon, R.drawable.argon_vector)
                ParticleDevice.ParticleDeviceType.BORON,
                ParticleDevice.ParticleDeviceType.B_SERIES -> Pair(R.string.product_name_boron, R.drawable.boron_vector)
                ParticleDevice.ParticleDeviceType.XENON,
                ParticleDevice.ParticleDeviceType.X_SERIES -> Pair(R.string.product_name_xenon, R.drawable.xenon_vector)
            ...
            val name = if (Py.truthy(device.name))
                device.name
            else
        ...
        private fun filter(query: String?, typeArrayList: List<ParticleDevice.ParticleDeviceType?>) {
            for (device in devices) {
...
private fun helpfulOrderDeviceComparator(): Comparator<ParticleDevice> {
    ...
    val nullComparator = Comparators.NullComparator<String>(false)
    ...
    val chain: Comparators.ComparatorChain<ParticleDevice>
    chain = Comparators.ComparatorChain(deviceOnlineStatusComparator, false)
