...
public class DeviceActionsHelper {
    ...
    public static boolean takeActionForDevice(int actionId, FragmentActivity activity, ParticleDevice device) {
        switch (actionId) {
            ...
            case R.id.action_device_flash_tinker:
                if (device.getDeviceType() == ParticleDevice.ParticleDeviceType.CORE) {
                    FlashAppHelper.flashKnownAppWithDialog(activity, device, ParticleDevice.KnownApp.TINKER);
                } else {
